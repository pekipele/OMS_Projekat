...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
		...
		private void SetTerminalRefernces(Dictionary<DMSType, ResourceDescription> updates)
        {
			// Fix this for test
			//for (int i = 0; i < updates[DMSType.TERMINAL].Properties.Count; i++)
			//{
			//    if (updates[DMSType.TERMINAL].Properties[i].Id == ModelCode.TERMINAL_CONDEQP)
			//    {
			//        updates[DMSType.TERMINAL].Properties[i].SetValue(updates[DMSType.POWERTRWINDING].Id);
			//    }
			//}
        }
